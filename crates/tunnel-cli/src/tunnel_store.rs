...
use tunnel_client::TunnelConfig;
...
impl TunnelStore {
    ...
    fn tunnel_path(&self, name: &str) -> PathBuf {
    ...
    pub fn save(&self, tunnel: &StoredTunnel) -> Result<()> {
        ...
        let path = self.tunnel_path(&tunnel.name);
    ...
    pub fn load(&self, name: &str) -> Result<StoredTunnel> {
        ...
        let path = self.tunnel_path(name);
        let json =
    ...
    pub fn exists(&self, name: &str) -> bool {
        Self::validate_name(name).is_ok() && self.tunnel_path(name).exists()
    }
...
mod tests {
    ...
    use tunnel_client::ProtocolConfig;
