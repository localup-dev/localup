...
mod routing_tests {
    ...
    fn test_tcp_route_lookup() {
        ...
        assert_eq!(found.tunnel_id, "test-tunnel");
    }
    ...
    fn test_http_route_lookup() {
        ...
        let target = RouteTarget {
            tunnel_id: "web-tunnel".to_string(),
        ...
        let found = registry.lookup(&key).unwrap();
        assert_eq!(found.tunnel_id, "web-tunnel");
    }
    ...
    fn test_sni_route_lookup() {
        ...
        let found = registry.lookup(&key).unwrap();
        assert_eq!(found.tunnel_id, "db-tunnel");
    }
