...
fn create_test_server_config(test_name: &str) -> Arc<QuicConfig> {
    ...
    let temp_dir = env::temp_dir().join(format!("localup-test-{}", test_name));
...
async fn test_user_provided_subdomain() {
    ...
    let localup_id = "test-user-subdomain";
...
async fn test_auto_generated_subdomain() {
    ...
    let localup_id = "test-auto-subdomain";
        ...
        TunnelMessage::Connected { endpoints, .. } => {
            ...
            match &endpoint.protocol {
                Protocol::Http { subdomain } => {
                    ...
                    // Should be deterministic based on localup_id (usually 6-8 characters)
                    assert!(
...
/// Test that same localup_id gets same auto-generated subdomain (deterministic)
#[tokio::test(flavor = "multi_thread")]
async fn test_deterministic_auto_generation() {
    ...
    // Connect twice with same localup_id, expect same auto-generated subdomain
    ...
    assert_eq!(
        ...
        "Auto-generated subdomain should be deterministic for same localup_id"
    );
    info!(
