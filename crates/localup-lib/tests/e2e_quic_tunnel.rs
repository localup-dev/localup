...
async fn test_quic_localup_message_flow() {
    ...
    let server_task: tokio::task::JoinHandle<Result<usize, String>> = tokio::spawn(async move {
        ...
        match msg {
            TunnelMessage::Connect { localup_id, .. } => {
                info!("Server: received Connect for tunnel {}", localup_id);
    ...
    let connect_msg = TunnelMessage::Connect {
        localup_id: "test-tunnel-123".to_string(),
    ...
    match response {
        TunnelMessage::Connected {
            ...
            info!("Client: tunnel registered as {}", localup_id);
