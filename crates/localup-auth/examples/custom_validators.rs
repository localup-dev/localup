...
use localup_auth::{AuthError, AuthResult, AuthValidator};
...
impl Default for ApiKeyValidator {
    fn default() -> Self {
        ...
        valid_keys.insert(
            ...
            ApiKeyInfo {
                localup_id: "localup_user1_http_3000".to_string(),
        ...
        valid_keys.insert(
            ...
            ApiKeyInfo {
                localup_id: "localup_user2_tcp_5432".to_string(),
...
impl AuthValidator for ApiKeyValidator {
    async fn validate(&self, token: &str) -> Result<AuthResult, AuthError> {
        match self.valid_keys.get(token) {
            Some(info) => Ok(AuthResult::new(info.localup_id.clone())
...
impl AuthValidator for DatabaseValidator {
    async fn validate(&self, token: &str) -> Result<AuthResult, AuthError> {
        ...
        if token.starts_with("db_") {
            ...
            let localup_id = format!("localup_{}_http_3000", user_id);

            // Mock database lookup result
            Ok(AuthResult::new(localup_id)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match api_validator.validate("sk_test_123456").await {
        Ok(result) => println!(
            "   ✅ Valid: localup_id={}, user_id={:?}, protocols={:?}",
            result.localup_id, result.user_id, result.allowed_protocols
        ),
    ...
    match multi_validator.validate("sk_test_123456").await {
        Ok(result) => println!("   ✅ API key accepted: {}", result.localup_id),
        Err(e) => println!("   ❌ Error: {}", e),
    }

    // Try database token (should succeed with second strategy)
    match multi_validator.validate("db_user456").await {
        Ok(result) => println!("   ✅ DB token accepted: {}", result.localup_id),
        Err(e) => println!("   ❌ Error: {}", e),
    }
    ...
    println!("async fn handle_localup_connection(");
    ...
    println!("    // Register tunnel with auth_result.localup_id");
    println!("}}");
    println!("```");
