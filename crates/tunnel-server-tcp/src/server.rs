...
impl TcpServer {
    ...
    async fn handle_http_connection(
        ...
        if target.target_addr.starts_with("tunnel:") {
            ...
            if let Some(ref manager) = tunnel_manager {
                // Forward through tunnel
                return Self::handle_tunnel_request(
                    client_socket,
                    manager.clone(),
                    pending_requests,
                    tunnel_id,
    ...
    async fn handle_tunnel_request(
        ...
        debug!("Forwarding request through tunnel: {}", tunnel_id);
            ...
            Ok(Ok(Some(TunnelMessage::HttpResponse {
                ...
                if let Some(ref db_conn) = db {
                    ...
                    let captured_request =
                        tunnel_relay_db::entities::captured_request::ActiveModel {
                            id: sea_orm::Set(request_id.clone()),
                            tunnel_id: sea_orm::Set(tunnel_id.to_string()),
                    ...
                    if let Err(e) = tunnel_relay_db::entities::prelude::CapturedRequest::insert(
                        captured_request,
                    )
                    .exec(db_conn)
                    .await
