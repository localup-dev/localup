...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Start server
    let server = ApiServer::new(config, tunnel_manager);
    server.start().await?;
...
```rust
use tunnel_api::run_api_server;

// Simple way to start API server
run_api_server("0.0.0.0:8080".parse()?, tunnel_manager).await?;
```
...
```bash
# Build
cargo build -p tunnel-api

# Test
cargo test -p tunnel-api

# Check OpenAPI generation
cargo test -p tunnel-api test_openapi_generation
```
