...
pub async fn list_tunnels(
    ...
    let tunnel_ids = state.tunnel_manager.list_tunnels().await;

    let mut tunnels = Vec::new();

    for tunnel_id in tunnel_ids {
        if let Some(endpoints) = state.tunnel_manager.get_endpoints(&tunnel_id).await {
            let tunnel = Tunnel {
                id: tunnel_id.clone(),
                endpoints: endpoints
                    .iter()
                    .map(|e| TunnelEndpoint {
                        protocol: match &e.protocol {
                            tunnel_proto::Protocol::Http { subdomain } => TunnelProtocol::Http {
                                subdomain: subdomain
                                    .clone()
                                    .unwrap_or_else(|| "unknown".to_string()),
                            },
                            tunnel_proto::Protocol::Https { subdomain } => TunnelProtocol::Https {
                                subdomain: subdomain
                                    .clone()
                                    .unwrap_or_else(|| "unknown".to_string()),
                            },
                            tunnel_proto::Protocol::Tcp { port } => {
                                TunnelProtocol::Tcp { port: *port }
                            }
                            tunnel_proto::Protocol::Tls {
                                port: _,
                                sni_pattern,
                            } => TunnelProtocol::Tls {
...
pub async fn get_tunnel(
    ...
    if let Some(endpoints) = state.tunnel_manager.get_endpoints(&id).await {
        let tunnel = Tunnel {
            id: id.clone(),
            endpoints: endpoints
                .iter()
                .map(|e| TunnelEndpoint {
                    protocol: match &e.protocol {
                        ...
                        tunnel_proto::Protocol::Tcp { port } => TunnelProtocol::Tcp { port: *port },
                        tunnel_proto::Protocol::Tls {
                            port: _,
                            sni_pattern,
                        } => TunnelProtocol::Tls {
...
pub async fn delete_tunnel(
    ...
    state.tunnel_manager.unregister(&id).await;
...
#[utoipa::path(
    ...
    path = "/api/requests",
    params(
        ("tunnel_id" = Option<String>, Query, description = "Filter by tunnel ID"),
...
pub async fn list_requests(
    ...
    use tunnel_relay_db::entities::captured_request::Column;
    use tunnel_relay_db::entities::prelude::*;
    ...
    let captured: Vec<tunnel_relay_db::entities::captured_request::Model> =
...
pub async fn list_tcp_connections(
    ...
    use sea_orm::{ColumnTrait, Condition, EntityTrait, PaginatorTrait, QueryFilter, QueryOrder};
    use tunnel_relay_db::entities::captured_tcp_connection::Column;
    use tunnel_relay_db::entities::prelude::*;
    ...
    // Get page of results
    let page_num = offset / limit;
    let captured: Vec<tunnel_relay_db::entities::captured_tcp_connection::Model> =
        paginator.fetch_page(page_num as u64).await.map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
