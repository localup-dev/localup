...
use tunnel_transport::{TransportConfig, TransportError, TransportResult, TransportSecurityConfig};
...
impl QuicConfig {
    ...
    pub fn server_ephemeral() -> TransportResult<Self> {
        ...
        let cert_path = temp_dir.join(format!("tunnel-quic-test-{}.crt", unique_id));
        let key_path = temp_dir.join(format!("tunnel-quic-test-{}.key", unique_id));
