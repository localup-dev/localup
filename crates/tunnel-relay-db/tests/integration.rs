...
use tunnel_relay_db::{connect, entities::captured_request, migrate};
...
async fn test_create_captured_request() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-1".to_string()),
...
async fn test_read_captured_request() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-2".to_string()),
...
async fn test_update_captured_request_with_response() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-3".to_string()),
...
async fn test_delete_captured_request() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-4".to_string()),
...
#[tokio::test]
async fn test_query_by_tunnel_id() {
    let db = setup_test_db().await;
    ...
    for i in 1..=3 {
        let request = captured_request::ActiveModel {
            ...
            tunnel_id: Set("tunnel-query-test".to_string()),
            method: Set("GET".to_string()),
    ...
    let other_request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("other-tunnel".to_string()),
        method: Set("GET".to_string()),
    ...
    let requests = captured_request::Entity::find()
        .filter(captured_request::Column::TunnelId.eq("tunnel-query-test"))
        .all(&db)
        .await
        .expect("Failed to query");

    assert_eq!(requests.len(), 3);
    assert!(requests.iter().all(|r| r.tunnel_id == "tunnel-query-test"));
}
...
async fn test_query_by_status_code() {
    ...
    for (i, status) in statuses.iter().enumerate() {
        let request = captured_request::ActiveModel {
            ...
            tunnel_id: Set("tunnel-status-test".to_string()),
            method: Set("GET".to_string()),
            path: Set("/".to_string()),
            host: Set(None),
            headers: Set("[]".to_string()),
...
async fn test_request_with_large_body() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-5".to_string()),
        method: Set("POST".to_string()),
...
async fn test_concurrent_inserts() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let request = captured_request::ActiveModel {
                ...
                tunnel_id: Set("tunnel-concurrent".to_string()),
                method: Set("GET".to_string()),
                path: Set(format!("/api/item/{}", i)),
                host: Set(None),
                headers: Set("[]".to_string()),
    ...
    let count = captured_request::Entity::find()
        .filter(captured_request::Column::TunnelId.eq("tunnel-concurrent"))
        .count(&db)
        .await
...
async fn test_latency_calculation() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-6".to_string()),
        method: Set("GET".to_string()),
...
async fn test_headers_json_encoding() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-7".to_string()),
        method: Set("POST".to_string()),
...
async fn test_nullable_fields() {
    ...
    let request = captured_request::ActiveModel {
        ...
        tunnel_id: Set("tunnel-8".to_string()),
        method: Set("GET".to_string()),
        path: Set("/".to_string()),
        host: Set(None),
        headers: Set("[]".to_string()),
