...
#[cfg(feature = "db-metrics")]
use tunnel_relay_db::entities::{captured_request, captured_tcp_connection};
...
impl DbMetricsStore {
    ...
    pub async fn new(db: DatabaseConnection, tunnel_id: String) -> Result<Self, sea_orm::DbErr> {
        Ok(Self {
            db: Arc::new(db),
            tunnel_id: Arc::new(RwLock::new(tunnel_id)),
        })
    }

    /// Update the tunnel ID (when reconnecting)
    pub async fn set_tunnel_id(&self, tunnel_id: String) {
        let mut tid = self.tunnel_id.write().await;
        *tid = tunnel_id;
    }
    ...
    pub async fn get_http_metrics(
        ...
        let models = captured_request::Entity::find()
            .filter(captured_request::Column::TunnelId.eq(tunnel_id))
    ...
    pub async fn get_tcp_metrics(
        ...
        let models = captured_tcp_connection::Entity::find()
            .filter(captured_tcp_connection::Column::TunnelId.eq(tunnel_id))
    ...
    pub async fn clear_metrics(&self) -> Result<(), sea_orm::DbErr> {
        ...
        captured_request::Entity::delete_many()
            .filter(captured_request::Column::TunnelId.eq(tunnel_id.clone()))
            .exec(self.db.as_ref())
            .await?;
