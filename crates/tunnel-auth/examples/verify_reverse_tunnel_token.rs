...
//!
//! Usage:
//!   cargo run --example verify_reverse_tunnel_token
...
fn main() {
    ...
    // Test validation
    match decoded.validate_reverse_tunnel_access("any-agent", "any-host:1234") {
    ...
    match decoded.validate_reverse_tunnel_access("agent-1", "any-host:1234") {
        Ok(()) => println!("   ✅ Access validated: agent-1 allowed"),
        Err(e) => println!("   ❌ Access denied: {}", e),
    }

    // Test validation - disallowed agent
    match decoded.validate_reverse_tunnel_access("agent-3", "any-host:1234") {
    ...
    match decoded.validate_reverse_tunnel_access("any-agent", "192.168.1.100:8080") {
        Ok(()) => println!("   ✅ Access validated: 192.168.1.100:8080 allowed"),
        Err(e) => println!("   ❌ Access denied: {}", e),
    }

    // Test validation - disallowed address
    match decoded.validate_reverse_tunnel_access("any-agent", "10.0.0.99:1234") {
    ...
    for (agent, addr, expected) in test_cases {
        let result = decoded.validate_reverse_tunnel_access(agent, addr);
